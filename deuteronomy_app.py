# deuteronomy_app.py — MM Deuteronomy Algo vs Competitors — Live Tracking
...
# ---------- Files ----------
PORTFOLIO_PATH = Path("mm_deuteronomy_algo.csv")           # cols: Ticker, Amount, Date, Action
MAP_PATH = Path("deuteronomy_ticker_map.csv")              # cols: User Ticker, Resolved Ticker [, Currency]
PARQUET_PATH = Path("deuteronomy_prices_cache.parquet")    # written daily by GitHub Actions
COMPETITORS = ["SPY", "QQQ"]                               # fixed competitors

EXPECTED_UPDATE_UTC = "22:25 UTC"  # matches the GitHub Action cron

# ---------- Page ----------
st.set_page_config(page_title="MM Deuteronomy Algo vs Competitors — Live Tracking", layout="wide")
st.title("MM Deuteronomy Algo vs Competitors — Live Tracking")
...
def build_chart(benchmarks_df: pd.DataFrame, start_date: pd.Timestamp) -> go.Figure:
    ...
    main_groups = [g for g in groups if g[0] == "MM Deuteronomy"]
    other_groups = [g for g in groups if g[0] != "MM Deuteronomy"]
    ordered_groups = main_groups + other_groups
...
# ---------- Main ----------
try:
    trades = load_trades(PORTFOLIO_PATH)
...
if not PARQUET_PATH.exists():
    st.error("Missing deuteronomy_prices_cache.parquet. The GitHub Action must write it first.")
    st.stop()
...
try:
    ...
    st.error(f"Failed to load deuteronomy_prices_cache.parquet: {e}")
    st.stop()
...
portfolio_df["series"] = "MM Deuteronomy"
...
st.download_button(
    ...
    file_name=f"mm_deuteronomy_vs_competitors_{datetime.utcnow().date().isoformat()}.html",
    data=html_bytes,
    mime="text/html",
)
